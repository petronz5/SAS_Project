...
public class EventInfo implements EventItemInfo {
    ...
    public int getDiners() {
        if (this.services != null && !this.services.isEmpty()) {
            return this.services.get(0).getDiners();  // Prende il numero di partecipanti dal primo servizio
        }
        return this.participants; // Se non ci sono servizi, usa il numero di partecipanti dell'evento
    }
    ...
    public static ArrayList<EventInfo> loadAllEventInfo() {
        ...
        for (EventInfo e : all) {
            ...
            for (ServiceInfo service : e.services) {
                service.setReferredEvent(e); // Imposta il riferimento all'evento
            }
        }
        return all;
    }
